...
export default function StudyPanel({ onExit }: StudyPanelProps) {
  ...
  const handleCompleteAndNext = async () => {
    ...
    try {
      ...
      setProgress((prev) => {
        ...
        if (exists) {
          return prev.map((p) =>
            p.lessonId === selectedLesson.id ? { ...p, completed: 1 } : p
          )
        } else {
          return [
            ...prev,
  ...
  if (loading)
    return (
      <div className={`flex-1 flex items-center justify-center bg-[#1c1c1e] text-white ${styles.studyPanel}`}>
  ...
  return (
    <div className={`flex h-screen w-screen bg-[#1c1c1e] text-[#a1a1aa] font-sans overflow-hidden ${styles.studyPanel}`}>
      {/* Sidebar */}
      <div
        className={`flex flex-col bg-[#212124] border-r border-[#2c2c2e] transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-[320px]'}`}
      >
        {/* Brand Header */}
        {!isCollapsed ? (
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-3 custom-scrollbar">
          <div className="space-y-1">
            {modules.map((mod) => {
              ...
              const trainings = mod.lessons.filter(
                (l) => l.type === 'training' || l.type === 'quiz'
              )
              const otherLessons = mod.lessons.filter(
                (l) => l.type !== 'training' && l.type !== 'quiz'
              )
              ...
              const isActiveInModule = mod.lessons.some(
                ...
                <div key={mod.id} className="mb-2">
                  ...
                  >
                    <div className="flex items-center gap-3 truncate">
                      ...
                      {!isCollapsed && (
                        <span
                          className={`font-medium text-sm truncate ${isExpanded ? 'text-blue-400' : 'text-gray-300'}`}
                        >
                          {mod.title}
                        </span>
                      )}
                          ...
                          return (
                            <div>
                              ...
                              >
                                ...
                                <Minus
                                  size={14}
                                  className={isSecExpanded ? 'opacity-100' : 'opacity-40'}
                                />
                              </button>
                              {isSecExpanded && (
                                  ...
                                  {trainings.map((lesson) => {
                                    ...
                                    return (
                                      ...
                                      >
                                        <Check
                                          size={14}
                                          className={`shrink-0 ${completed ? 'text-emerald-500' : 'text-gray-600'}`}
                                        />
                                        <span className="text-left leading-tight pr-2 truncate">
                                          {lesson.title}
                                        </span>
                                      </button>
                                    )
                      ...
                      {otherLessons.map((lesson) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <Check
                              size={12}
                              className={`shrink-0 ml-2 ${completed ? 'text-emerald-500' : 'text-gray-600 opacity-0'}`}
                            />
                          </button>
                        )
                      })}
              ...
              >
                ...
                <span>
                  {isLessonCompleted(selectedLesson.id)
                    ? 'Наступний блок →'
                    : 'Приступити до наступного блоку ✓'}
                </span>
              </button>
            </div>
